...
const artistPaginatedRequestValidator = joi.object({
  name: joi.string().optional().trim().min(1).max(150).regex(findRegex("artist_name", artistValidations)).messages({
  ...
  country: joi.string().optional().trim().length(3).uppercase().regex(findRegex("country_code", artistValidations)).messages({
  ...
  formationYear: joi.number().optional().integer().min(1500).max(new Date().getFullYear()).messages({
