...
const artistUpdateSchema = Joi.object({
  artist_name: Joi.string().optional().trim().min(2).max(150).regex(findRegex("artist_name", artistValidations)).messages({
  ...
  biography: Joi.string().optional().trim().max(2000).allow(null, "").regex(findRegex("artist_biography", artistValidations)).messages({
  ...
  formation_year: Joi.number().optional().integer().positive().min(1600).max(new Date().getFullYear()).messages({
  ...
  country_code: Joi.string().optional().alphanum().min(2).max(6).regex(findRegex("country_code", artistValidations)).messages({
