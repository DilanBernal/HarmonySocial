...
const registerSchema = joi.object({
  full_name: joi.string().trim().required().min(3).max(150).pattern(userFindRegex("fullNameRegex")).messages({
  ...
  username: joi.string().trim().required().min(3).max(50).pattern(userFindRegex("usernameRegex")).messages({
  ...
  email: joi.string().trim().required().min(3).max(90).pattern(userFindRegex("emailRegex")).messages({
  ...
  password: joi.string().required().min(8).max(128).pattern(userFindRegex("passwordRegex")).messages({
  ...
  profile_image: joi
    ...
    .allow("", null)
    .default("avatar1")
    .pattern(userFindRegex("profileImageRegex")).messages({
