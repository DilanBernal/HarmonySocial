...
const registerSchema = joi.object({
  ...
  username: joi.string().required().trim().min(3).max(50).pattern(userFindRegex("usernameRegex")).messages({
  ...
  email: joi.string().required().trim().lowercase().min(3).max(90).pattern(userFindRegex("emailRegex")).messages({
  ...
  password: joi.string().required().trim().min(8).max(128).pattern(userFindRegex("passwordRegex")).messages({
...
}).required().options({ stripUnknown: true }).messages({
