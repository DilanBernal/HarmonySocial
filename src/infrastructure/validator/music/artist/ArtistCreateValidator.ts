...
const artistCreateSchema = Joi.object({
  artist_name: Joi.string().required().trim().min(2).max(150).regex(findRegex("artist_name", artistValidations)).messages({
  ...
  biography: Joi.string().optional().trim().max(2000).regex(findRegex("artist_biography", artistValidations)).messages({
  ...
  formation_year: Joi.number().required().positive().integer().min(1300).max(new Date().getFullYear()).messages({
