...
const userSearchParamsSchema = joi.object({
  ...
  full_name: joi.string().optional().trim().allow(null).min(3).max(150).pattern(userFindRegex("fullNameRegex")).messages({
  ...
  username: joi.string().optional().trim().uppercase().min(3).max(50).allow(null).pattern(userFindRegex("usernameRegex")).messages({
...
}).required().options({ stripUnknown: true, abortEarly: false }).messages({
