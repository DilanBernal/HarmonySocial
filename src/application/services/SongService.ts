...
export default class SongService {
  ...
  async search(query = "", page = 1, limit = 20): Promise<{ data: Song[]; total: number; page: number; limit: number }> {
  ...
  async getMine(userId: number, page = 1, limit = 20): Promise<{ data: Song[]; total: number; page: number; limit: number }> {
  ...
  async update(id: number, dto: SongUpdateDTO): Promise<Song | null> {
    ...
    if (dto.difficultyLevel !== undefined) updateData.difficultyLevel = dto.difficultyLevel as SongDifficultyLevel | null | undefined;
