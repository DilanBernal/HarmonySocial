  ...
  return (
    <View style={{ flex:1, backgroundColor:'#0b0c16', padding:16 }}>
      <View style={{ alignItems:'center' }}>
        <Image source={{ uri: song.artwork || 'https://placehold.co/200x200/png' }}
               style={{ width:160, height:160, borderRadius:12, marginBottom:12 }}/>
        <Text style={{ color:'#fff', fontSize:20, fontWeight:'700' }}>{song.title}</Text>
        {!!(song as any).artist && <Text style={{ color:'#9aa3b2', marginTop:4 }}>{(song as any).artist}</Text>}
      </View>

      <Pressable
        onPress={() => playSong({ title: song.title, audioUrl: song.audioUrl })}
        style={{ marginTop:20, backgroundColor:'#4f46e5', paddingVertical:10, borderRadius:8, alignItems:'center' }}>
